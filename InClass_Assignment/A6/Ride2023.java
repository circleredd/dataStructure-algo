...
public class Ride2023 {
	...
	public Integer findRide(Digraph g, Bag<Integer> friendLocations, Integer yourLocation, Integer destination) {
		BreadthFirstDirectedPaths bfs = new BreadthFirstDirectedPaths(g, friendLocations);
		int min = 1000;		//最小距離，初值設高一點
		int friend = -1;	//最後要回傳的朋友
		for(int w:friendLocations){
			bfs = new BreadthFirstDirectedPaths(g, w);
			for(int i:bfs.pathTo(destination)){			//迭代找該朋友到目的地的路上的點有沒有yourlocation，若有則更新最小值及friend
				if(i == yourLocation) {
					if(bfs.distTo(destination) < min) {
						min = bfs.distTo(destination);
						friend = w;
					}
				}
			}
		}
		if(min == 1000)	return -1;	//若min初值沒變表示沒有同時達成目標
		return friend;
	}
